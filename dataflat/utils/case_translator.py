...
class CustomCaseTranslator(BaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        logger.info(
            f"CustomCaseTranslator for {self.from_case.name} to {self.to_case.name} has been initiated"
        )
    ...
    def _pre_process_string(self, string: str) -> str:
        ...
        conv_string = re.sub(
            r"(?<=[A-Z])(?=[A-Z][a-z])", " ", conv_string
        )  # PascalCase to Pascal Case
        conv_string = re.sub(
            r"(?<=[a-z])(?=\d)", " ", conv_string
        )  # text123 to text 123
        conv_string = re.sub(
            r"(?<=\d)(?=[a-zA-Z])", " ", conv_string
        )  # 123text to 123 text
    ...
    def _normalize(self, string: str) -> list[str]:
        ...
        """
        split_char = (
            self.from_case.value if self.from_case.name in ("SNAKE", "KEBAB") else " "
        )
        replaced_strings = string.split(split_char)
        return [
            replaced_string.lower()
    ...
    def _camel(self, string: str) -> str:
        ...
        return "".join(
            [
                ...
                for index, word in enumerate(self._normalize(string))
            ]
        )
    ...
    def translate(self, string: str) -> str:
        """Receive a string and convert it to the desirable case."""
