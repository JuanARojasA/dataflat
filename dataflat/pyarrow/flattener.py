...
class CustomFlattener(BaseFlattener):
    ...
    def __unnest_struct_col(self, table: pa.Table, col: str) -> pa.Table:
    ...
    def __expand_structs(self, table: pa.Table, entity_name: str) -> pa.Table:
        ...
        while True:
            ...
            for col in struct_cols:
                if self.__is_blacklisted(entity_name, col):
                    ...
                    table = self.__unnest_struct_col(table, col)
        return table
    ...
    def __explode(
        ...
        lengths = pc.list_value_length(list_arr)  # pyrefly: ignore[missing-attribute]
        has_items = pc.greater(lengths.fill_null(0), 0)  # pyrefly: ignore[missing-attribute]
        ...
        if pa.types.is_struct(inner_type):
            ...
            # correctly by __expand_structs when __process_table recurses.
    ...
    def __process_table(
        ...
        table = self.__expand_structs(table, entity_name)
        ...
        for list_col in list_cols:
            ...
            child_table, index_arr = self.__explode(child_table, list_col, inner_type)
            child_table = child_table.append_column(
                pa.field("index", pa.int64()), index_arr
            )

            # Recurse into the child entity.
            self.__process_table(child_table, child_name, child_pk_cols, is_root=False)
        ...
        for col in [
            ...
            if c not in inherited_cols and self.__is_blacklisted(entity_name, c)
        ]:
            col_idx = table.schema.get_field_index(col)
            table = table.remove_column(col_idx)
    ...
    def flatten(
        ...
        self.__process_table(data, self.entity_name, root_inherited, is_root=True)
